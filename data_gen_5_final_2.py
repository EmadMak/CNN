...
NUM_AUGMENTATIONS_PER_CHAIN_TYPE = 75
...
NUM_NOISE_AUGMENTATIONS = 10

MAX_ALLOWED_MORPH_KERNEL_SIZE = 1
...
def apply_morphology(img: Image.Image, kernel_size: int, type: str) -> Image.Image:
    ...
    if type == "erode":
        ...
        morphed = img.filter(ImageFilter.MaxFilter(MAX_ALLOWED_MORPH_KERNEL_SIZE))
    else:
        return img 
...
def generate_augmentations(base_img: Image.Image, config: dict, num_images_per_chain: int) -> tuple[int, list[Image.Image]]:
    ...
    for length in range(1, MAX_CHAIN_LENGTH + 1):
        ...
        # Remove chains with consecutive identical groups
        filtered_chains = []
        for combo in chains_of_length:
            if len(combo) == len(set(combo)):
                filtered_chains.append(combo)

        all_chains.extend(filtered_chains)
        
    print(f"    - Generating augmentations for {len(all_chains)} unique chain types (L1 to L{MAX_CHAIN_LENGTH}), excluding consecutive duplicates.")
    
    for combo_keys in all_chains:
        # Create a descriptive directory name for this chain type
...
def augment_pipeline(base_images):
    ...
    # The number of unique chains is now dependent on the new filtering logic.
    # We can only give an estimate in the printout, or just remove the printout for simplicity.
    # Since the number of chains (340) was a constant before, I will keep the original constant 
    # for chains L1-L4 (4 + 4*3 + 4*3*3 + 4*3*3*3 = 4 + 12 + 36 + 108 = 160)
    # The new total unique chains is 160. I will update the printout below.
    
    
    print(f"\n--- Augmentation Pipeline Configuration ---")
    print(f"Total Unique Chains (L1-L{MAX_CHAIN_LENGTH}, no consecutive duplicates): 160")
    print(f"Noise Applied To Chain Lengths (L): {NOISE_APPLY_CHAIN_LENGTHS}")
    # Removed the expected input/output calculation as it's harder to calculate accurately with deduplication.
    print(f"-----------------------------------------\n")
