...
def generate_single_augmentations(base_img: Image.Image, config: dict, num_images: int) -> tuple[int, list[Image.Image]]:
    """
    ...
    Now returns the list of generated images in memory.
    """
    total_generated = 0
    base_name = base_img.base_name
    generated_images = [] # Collect images to return
    ...
    for key, group_functions in transformation_map.items():
        ...
        for i in range(num_images):
            ...
            # --- Store in memory for later noise application ---
            # Re-attach metadata lost during transformation
            current_img.base_name = base_img.base_name
            current_img.class_id_key = base_img.class_id_key
            # Tag used for traceability in the noise phase (e.g., Rr-4, S_s2y1)
            current_img.chained_tag = f"{key}{tag}" 
...
def generate_chained_augmentations(base_img: Image.Image, config: dict, num_images: int) -> tuple[int, list[Image.Image]]:
    ...
    with the full parameter tag attached as an attribute.
    """
    total_generated = 0
    base_name = base_img.base_name
    generated_images = []
...
def augment_pipeline(base_images):
    ...
    for base_img in base_images:
        ...
        # List to hold ALL geometrically transformed images for noise application
        ...
        # --- PHASE 1a: Single Augmentations (R, S, Z, M) ---
        print(f" -> Generating single-effect augmentations (R, S, Z, M)...")
        single_count, single_images = generate_single_augmentations(base_img, config, NUM_SINGLE_AUGMENTATIONS)
        total_images_generated += single_count
        all_transformed_images.extend(single_images)
        print(f"    - Generated {single_count} single-effect augmentations. ({len(all_transformed_images)} transformed images total)")

        # --- PHASE 1b: Combinatorial Augmentations (R+S+Z, etc.) ---
        ...
        all_transformed_images.extend(chained_images)
        print(f"    - Generated {chained_count} complex augmentations. ({len(all_transformed_images)} transformed images total)")
        
        # --- PHASE 1c: Noise on ALL Transformed Images (FIXED: Applies noise to all of 1a + 1b) ---
        print(f" -> Generating noise on ALL {len(all_transformed_images)} transformed images...")
        noisy_transformed_count = generate_noise_on_transformed(all_transformed_images, config, NUM_NOISE_AUGMENTATIONS)
        total_images_generated += noisy_transformed_count
        print(f"    - Generated {noisy_transformed_count} total noisy transformed augmentations. Saved to 'noise_on_transformed'.")
